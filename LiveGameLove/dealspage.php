...
if ($result = $mysqli->query($query)) {
 ...
 {                                                                                                   //this is basically repeated throughout
     echo "Error getting cars from the database: " . mysql_error()."<br>";                          //This page ONLY shows things with deals
 }
 while ($result_ar = mysqli_fetch_assoc($result)) {                        //This will make a sepparate thing to hold the information of a row for each row
    if(date("l")==$result_ar['DAY_DEAL']){
...
echo "<b>Brand: </b>" . $result_ar['BRAND']."<br>";                               //this is just like the stuff in gamingpc.php
...
if(date("l")==$result_ar['DAY_DEAL']){                                         //depending on the day deals will change
    ...
    echo "<b><span>Price Now:</b> $" . number_format($result_ar['PRICE']*$deal,2)."</span><br>";                       //this shows the original price and the deal price
}
else{
    $deal=1;
...
<?php
include 'db.php';
$query = "SELECT * FROM keyboards ORDER BY PRICE";
if ($result = $mysqli->query($query)) {
    // Don't do anything if successful.
 }
 else
 ...
 while ($result_ar = mysqli_fetch_assoc($result)) {  
    if(date("l")==$result_ar['DAY_DEAL']){                      //This will make a sepparate thing to hold the information of a row for each row
...
echo "<b>Purpose:</b> " . $result_ar['PURPOSE']."<br>";
echo "<b>LED:</b> " . $result_ar['LED']."<br>";
echo "<b>Wireless:</b> " . $result_ar['WIRELESS']."<br>";
if(date("l")==$result_ar['DAY_DEAL']){                                         //depending on the day deals will change
    $deal=.9;
    echo"<b>Price Was:</b> $". number_format($result_ar['PRICE'],2). "  ";
...
include 'db.php';
$query = "SELECT * FROM PC ORDER BY PRICE";
if ($result = $mysqli->query($query)) {
    // Don't do anything if successful.
 }
 else
...
<?php
include 'db.php';
$query = "SELECT * FROM GRAPHICS_CARD ORDER BY PRICE";
if ($result = $mysqli->query($query)) {
    // Don't do anything if successful.
 }
 else
...
echo    "<div class='info'>";
echo        "<a href='#'>Add To Cart</a>";
echo  "<h1>".$result_ar['BRAND'] ." ". $result_ar['NAME']." ". $result_ar['VERSION']. "</h1><br>";
echo "<h2>Specifications: </h2><br>";
echo "<div class='.specs'>";
echo "<b>Type:</b> " . $result_ar['NAME']."<br>";
echo "<b>Version:</b> " . $result_ar['VERSION']."<br>";
if(date("l")==$result_ar['DAY_DEAL']){                                         //depending on the day deals will change
    $deal=.9;
    echo"<b>Price Was:</b> $". number_format($result_ar['PRICE'],2). "  ";
    echo "<b><span>Price Now:</b> $" . number_format($result_ar['PRICE']*$deal,2)."</span><br>";                                 
...
<?php
include 'db.php';
$query = "SELECT * FROM gamingmouse ORDER BY PRICE";
if ($result = $mysqli->query($query)) {
    // Don't do anything if successful.
 }
 else
...
echo "<b>Brand:</b> " . $result_ar['BRAND']."<br>";
echo "<b>Wireless:</b> " . $result_ar['WIRELESS']."<br>";
echo "<b>Max DPI:</b> " . $result_ar['MAX_DPI']." DPI<br>";
echo "<b>Number of Buttons:</b> " . $result_ar['BUTTONS']."<br>";
echo "<b>Color:</b> " . $result_ar['COLOR']."<br>";
if(date("l")==$result_ar['DAY_DEAL']){                                         //depending on the day deals will change
    $deal=.9;
    echo"<b>Price Was:</b> $". number_format($result_ar['PRICE'],2). "  ";
    echo "<b><span>Price Now:</b> $" . number_format($result_ar['PRICE']*$deal,2)."</span><br>";                                 
